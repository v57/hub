...
test("resolves the pid file inside the launcher directory", () => {
  expect(resolvePidFilePath("/Users/dimas/Hub/Launcher", path.posix)).toBe("/Users/dimas/Hub/Launcher/.hub-launcher.pid");
});
...
test("imports a launch configuration after backing up the current launcher", async () => {
  ...
  try {
    ...
    await Bun.write(resolveLaunchJsonPath(launcherDirectory), `${JSON.stringify(originalLaunchConfiguration, null, 2)}\n`);
    ...
    expect(await Bun.file(resolveLaunchJsonPath(launcherDirectory)).text()).toBe(`${JSON.stringify(importedLaunchConfiguration)}\n`);
    expect(await Bun.file(path.join(backupDirectory, "launch.json")).text()).toBe(`${JSON.stringify(originalLaunchConfiguration, null, 2)}\n`);
    expect(messages).toContain(`Hub Launcher backed up to ${backupDirectory}`);
    expect(messages).toContain(`Hub Launcher launch configuration imported to ${resolveLaunchJsonPath(launcherDirectory)}`);
  } finally {
    await rm(homeDirectory, { recursive: true, force: true });
  }
});
...
test("previews an imported launch configuration without replacing it", async () => {
  ...
  try {
    ...
    expect(messages).toEqual([
      JSON.stringify(importedLaunchConfiguration, null, 2),
    ]);
