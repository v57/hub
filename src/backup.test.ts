...
test("removes a backup by id and removes all backups", async () => {
  ...
  try {
    ...
    expect(await Bun.file(path.join(resolveBackupDirectory(homeDirectory, "2026-04-03T00-00-00.000Z"), "index.ts")).exists()).toBe(false);
    expect(await Bun.file(path.join(resolveBackupDirectory(homeDirectory, "2026-04-04T00-00-00.000Z"), "index.ts")).exists()).toBe(true);
    ...
    expect(await Bun.file(path.join(resolveBackupsDirectory(homeDirectory), "2026-04-04T00-00-00.000Z", "index.ts")).exists()).toBe(false);
  } finally {
    await rm(homeDirectory, { recursive: true, force: true });
  }
});
